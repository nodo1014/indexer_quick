document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function initializePage() {
    try {
      ...
      console.log(`페이지 초기화 시도 ${initializationAttempts}/${MAX_ATTEMPTS}`);
      ...
      // 결과 렌더링 이벤트 핸들러 등록
      document.addEventListener('searchResultsRendered', handleSearchResultsRendered);
      ...
      console.error(`페이지 초기화 중 오류 발생(시도 ${initializationAttempts}/${MAX_ATTEMPTS}):`, error);
      
      // 최대 시도 횟수에 도달하면 로딩 화면 제거하고 오류 메시지 표시
      if (initializationAttempts >= MAX_ATTEMPTS) {
        console.error("최대 초기화 시도 횟수에 도달했습니다. 사용자에게 오류 표시");
        ...
        if (contentElement) {
          ...
          contentElement.innerHTML = `
            <div class="text-center p-8">
              ...
              <pre class="text-xs text-left bg-gray-100 p-2 rounded overflow-auto max-h-40">${error.stack || '스택 정보 없음'}</pre>
  ...
  async function loadModules() {
    try {
      ...
      const TagManagerModule = await import('./components/tag-manager.js').catch(e => {
        console.error("TagManager 모듈 로드 실패:", e);
        throw e;
      });
      
      const BookmarkManagerModule = await import('./components/bookmark-manager.js').catch(e => {
        console.error("BookmarkManager 모듈 로드 실패:", e);
        throw e;
      });
      
      const SearchResultsViewModule = await import('./components/search-results-view.js').catch(e => {
  ...
  async function initVideoPlayer() {
    try {
      ...
      try {
        const VideoPlayerModule = await import('./components/video-components/video-player.js');
        const VideoPlayer = VideoPlayerModule.default || VideoPlayerModule.VideoPlayer;
    ...
    if (searchForm) {
      searchForm.addEventListener("submit", function (e) {
        ...
        const languageSelector = document.querySelector(
          'input[name="search_lang"]:checked'
        )?.value || 'en';
        ...
        // 태그 검색 여부 확인
        const searchInTags = document.getElementById("search-tags")?.checked || false;
      ...
      try {
        if (window.SearchAPI && typeof window.SearchAPI.search === 'function') {
          ...
          fetch(`/api/search?${params.toString()}`)
            ...
            .then(data => {
              if (!data.success) {
                throw new Error(data.error || "검색 처리 중 오류가 발생했습니다.");
              }
        ...
        if (searchResultsView) {
          searchResultsView.updateUIForLoading(false);
          searchResultsView.displayError(`검색 중 예외가 발생했습니다: ${error.message}`);
        }
      }
    }
