...
window.SearchAPI = {
  ...
  search: async function (query, options = {}) {
    try {
      ...
      // 검색 방식(LIKE 또는 FTS) 추가
      if (!options.hasOwnProperty("search_method")) {
        // 기본값은 LIKE 검색
        url.searchParams.append("search_method", "like");
      }
      ...
      if (!response.ok) {
        throw new Error(
          `검색 API 오류: ${response.status} ${response.statusText}`
        );
      }
  ...
  getTags: async function (mediaPath) {
    try {
      const response = await fetch(
        `/api/tags/${encodeURIComponent(mediaPath)}`
      );
      if (!response.ok) {
        throw new Error(
          `태그 API 오류: ${response.status} ${response.statusText}`
        );
      }
  ...
  addTag: async function (mediaPath, startTime, tag) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `태그 추가 API 오류: ${response.status} ${response.statusText}`
        );
      }
  ...
  removeTag: async function (mediaPath, startTime, tag) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `태그 삭제 API 오류: ${response.status} ${response.statusText}`
        );
      }
  ...
  toggleBookmark: async function (mediaPath, startTime, bookmarked) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `북마크 API 오류: ${response.status} ${response.statusText}`
        );
      }
  ...
  getBookmarkStatus: async function (mediaPath, startTime) {
    try {
      const response = await fetch(
        `/api/bookmarks/status?media_path=${encodeURIComponent(
          mediaPath
      ...
      if (!response.ok) {
        throw new Error(
          `북마크 상태 API 오류: ${response.status} ${response.statusText}`
        );
      }
